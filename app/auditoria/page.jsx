...
function Input(props){ return <input {...props} className={"w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 " + (props.className||"")} /> }
function Select(props){ return <select {...props} className={"w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 " + (props.className||"")} /> }
...
export default function AuditoriaPage(){
  ...
  // filtros
  const [tbl, setTbl] = useState("");     // tabla
  ...
  async function load(){
    setLoading(true);
    let query = supabase.from("audit_log_view")
  ...
  useEffect(() => { load(); /* eslint-disable-next-line */ }, [tbl, op, limit]);
          ...
          <div className="grid gap-3 md:grid-cols-5">
            <div>
              ...
              <Select value={tbl} onChange={(e)=>setTbl(e.target.value)}>
                {allTables.map(t => <option key={t} value={t}>{t || "Todas"}</option>)}
              </Select>
            </div>
            <div>
            ...
            <div className="md:col-span-2">
              ...
              <Input placeholder="Texto, email, id, etc." value={q} onChange={(e)=>setQ(e.target.value)} />
            </div>
            <div>
                  ...
                  <tr key={r.id} className="border-t align-top">
                    ...
                    <td className="p-2">
                      <details className="rounded border bg-gray-50 p-2">
                        <summary className="cursor-pointer text-xs text-gray-600">Ver JSON</summary>
                        <pre className="mt-2 max-w-[80ch] overflow-auto text-xs">{JSON.stringify({ old: r.old_data, new: r.new_data }, null, 2)}</pre>
                      </details>
                    </td>
                  </tr>
                ))}
