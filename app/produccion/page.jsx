...
export default function ProduccionPage() {
  ...
  async function createRun(e) {
    ...
    if (!payload.machine_id) {
      setCreating(false);
      return alert("Selecciona la máquina.");
    }
    if (!payload.started_at || !payload.ended_at) {
      setCreating(false);
      return alert("Indica inicio y fin.");
    }
    if (!payload.planned_time_min || payload.planned_time_min <= 0) {
      setCreating(false);
      return alert("Tiempo planificado > 0.");
    }
    if (!payload.ideal_cycle_time_sec || payload.ideal_cycle_time_sec <= 0) {
      setCreating(false);
      return alert("Tiempo de ciclo ideal > 0.");
    }
    ...
    if (error) {
      setCreating(false);
      return alert("Error creando corrida: " + error.message);
    }
    ...
    if (runId && payload.recipe_id && batches > 0) {
      ...
      if (!ei && (ings?.length || 0) > 0) {
        ...
        const { error: emv } = await supabase
  ...
  function exportRunsCSV() {
    ...
    const rows = filtered.map((r) => [
      ...
      r.machines?.name || "",
      r.recipes?.name || "",
      Number(r.planned_time_min ?? 0),
      Number(r.downtime_min ?? 0),
      Number(r.ideal_cycle_time_sec ?? 0),
      Number(r.good_count ?? 0),
      Number(r.reject_count ?? 0),
      ((r._oee?.A || 0) * 100).toFixed(2),
      ((r._oee?.P || 0) * 100).toFixed(2),
      ((r._oee?.Q || 0) * 100).toFixed(2),
      ((r._oee?.OEE || 0) * 100).toFixed(2),
    ]);
    downloadCSV("produccion_corridas.csv", headers, rows);
  }
          ...
          <form onSubmit={createRun} className="grid gap-3 md:grid-cols-4">
            <div>
              ...
              <Select
                ...
                onChange={(e) =>
                  setForm((s) => ({ ...s, machine_id: e.target.value }))
                }
                required
              >
            ...
            <div>
              ...
              <Select
                ...
                onChange={(e) =>
                  setForm((s) => ({ ...s, recipe_id: e.target.value }))
                }
              >
                <option value="">—</option>
                {recipes.map((r) => (
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((s) => ({ ...s, started_at: e.target.value }))
                }
                required
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                value={form.ended_at}
                onChange={(e) =>
                  setForm((s) => ({ ...s, ended_at: e.target.value }))
                }
                required
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((s) => ({ ...s, planned_time_min: e.target.value }))
                }
                required
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((s) => ({ ...s, downtime_min: e.target.value }))
                }
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((s) => ({ ...s, good_count: e.target.value }))
                }
                required
              />
            </div>
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setForm((s) => ({ ...s, reject_count: e.target.value }))
                }
              />
            </div>
            ...
            <div>
              ...
              <p className="mt-1 text-[11px] text-gray-500">
                1 batch = cantidades definidas en la receta. Se descuenta del
                inventario.
              </p>
            </div>
            ...
            <div className="md:col-span-4">
              <Button disabled={creating}>
                {creating ? "Guardando…" : "Registrar corrida"}
              </Button>
            </div>
          </form>
        </div>
        ...
        <div className="rounded-2xl bg-white p-6 shadow">
          <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <Button variant="ghost" onClick={exportRunsCSV}>
                Exportar CSV
              </Button>
            </div>
          </div>
              ...
              <tbody>
                {filtered.map((r) => (
                  <tr key={r.id} className="border-t">
                    ...
                    <td className="p-2">
                      {r.machines?.name}{" "}
                      <span className="text-gray-400">({r.machines?.code})</span>
                    </td>
