...
export default function MantenimientoPage() {
  ...
  async function loadAll() {
    setLoading(true);
    const { data: m } = await supabase.from("machines").select("id, code, name, location, is_active").order("created_at");
    setMachines(m || []);
      ...
      <div className="grid gap-6">
        <div className="rounded-2xl bg-white p-6 shadow">
          ...
          <div className="mb-4 flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <Input placeholder="Buscar por título, máquina, tipo o estado…" value={q} onChange={(e) => setQ(e.target.value)} />
              ...
              <tbody>
                {filtered.map((o) => (
                  <tr key={o.id} className="border-t">
                    ...
                    <td className="p-2"><span className="rounded-full bg-gray-100 px-2 py-1 text-xs capitalize">{o.status}</span></td>
                    <td className="p-2">
                ...
                {!loading && filtered.length === 0 && (
                  <tr><td className="p-3 text-gray-500" colSpan={7}>No hay mantenimientos aún.</td></tr>
                )}
              </tbody>
            </table>
          </div>
          ...
          {machines.length === 0 ? (
            <div className="rounded-lg border border-amber-300 bg-amber-50 p-4">
              ...
              <form onSubmit={createMachine} className="grid gap-3 md:grid-cols-3">
                ...
                <div className="md:col-span-3"><Button disabled={creatingMachine}>{creatingMachine ? "Creando…" : "Crear máquina"}</Button></div>
              </form>
            </div>
          ) : (
            <form onSubmit={createOrder} className="grid gap-3 md:grid-cols-3">
              ...
              <div className="md:col-span-3"><Button disabled={creating}>{creating ? "Guardando…" : "Programar"}</Button></div>
            </form>
          )}
        </div>
      </div>
