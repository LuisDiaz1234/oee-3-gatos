...
export default function MantenimientoPage() {
  ...
  async function loadAll() {
    ...
    const { data: o } = await supabase
      ...
      .select(
        "id, title, description, kind, status, priority, scheduled_at, created_at, machines (name, code)"
      )
  ...
  async function createOrder(e) {
    ...
    setCreating(false);
    if (error) return alert("Error creando mantenimiento: " + error.message);
    setForm({
      machine_id: "",
      kind: "preventivo",
      title: "",
  ...
  async function createMachine(e) {
    ...
    if (error) return alert("Error creando máquina: " + error.message);
    setNewMachine({ code: "", name: "", location: "" });
    await loadAll();
    if (data?.[0]?.id) setForm((f) => ({ ...f, machine_id: data[0].id }));
  }
  ...
  function exportCSV() {
    ...
    const rows = filtered.map((o) => [
      ...
      o.kind,
      o.priority,
      o.scheduled_at ? new Date(o.scheduled_at).toISOString() : "",
      o.status,
    ]);
    downloadCSV("mantenimientos.csv", headers, rows);
  }
              ...
              <tbody>
                {filtered.map((o) => (
                  <tr key={o.id} className="border-t">
                    ...
                    <td className="p-2">
                      {o.machines?.name}{" "}
                      <span className="text-gray-400">({o.machines?.code})</span>
                    </td>
                    ...
                    <td className="p-2">
                      <div className="flex gap-2">
                        <Button
                          variant="ghost"
                          onClick={() => quickStatus(o.id, "en_proceso")}
                        >
                          En proceso
                        </Button>
                        <Button
                          variant="ghost"
                          onClick={() => quickStatus(o.id, "completado")}
                        >
                          Completar
                        </Button>
                        <Button
                          variant="ghost"
                          onClick={() => quickStatus(o.id, "cancelado")}
                        >
                          Cancelar
                        </Button>
                      </div>
          ...
          {machines.length === 0 ? (
            <div className="rounded-lg border border-amber-300 bg-amber-50 p-4">
              <p className="mb-3 text-sm">
                No hay máquinas registradas. Crea una rápida:
              </p>
              <form onSubmit={createMachine} className="grid gap-3 md:grid-cols-3">
                <Input
                  placeholder="Código (ej. FERM-01)"
                  value={newMachine.code}
                  onChange={(e) =>
                    setNewMachine((s) => ({ ...s, code: e.target.value }))
                  }
                  required
                />
                <Input
                  placeholder="Nombre (ej. Fermentador 1)"
                  value={newMachine.name}
                  onChange={(e) =>
                    setNewMachine((s) => ({ ...s, name: e.target.value }))
                  }
                  required
                />
                <Input
                  ...
                  onChange={(e) =>
                    setNewMachine((s) => ({ ...s, location: e.target.value }))
                  }
                />
                <div className="md:col-span-3">
                  <Button disabled={creatingMachine}>
                    {creatingMachine ? "Creando…" : "Crear máquina"}
                  </Button>
                </div>
              </form>
            </div>
            ...
            <form onSubmit={createOrder} className="grid gap-3 md:grid-cols-3">
              ...
              <div className="md:col-span-3">
                ...
                <Input
                  placeholder="Notas / alcance (opcional)"
